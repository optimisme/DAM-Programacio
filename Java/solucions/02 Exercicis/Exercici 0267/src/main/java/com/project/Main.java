...
public class Main {
    ...
    public static List<String> ordenaParaulesPerLongitud(List<String> paraules, boolean ascendent) {
        List<String> paraulesOrdenades = new ArrayList<>(paraules);
        if (ascendent) {
            paraulesOrdenades.sort(Comparator.comparingInt(String::length));
        } else {
            paraulesOrdenades.sort(Comparator.comparingInt(String::length).reversed());
        }
