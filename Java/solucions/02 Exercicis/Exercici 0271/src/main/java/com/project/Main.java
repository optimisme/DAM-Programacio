...
public class Main {
    ...
    public static Object[] mitjanesArray(double[][] array) {
        double[] mitjanes = new double[array.length];
        double sumaTotal = 0;
        int totalElements = 0;
        ...
        for (int i = 0; i < array.length; i++) {
            ...
            mitjanes[i] = sumaArray / array[i].length;
            sumaTotal += sumaArray;
            totalElements += array[i].length;
        }

        double mitjanaTotal = sumaTotal / totalElements;
        return new Object[]{mitjanes, mitjanaTotal};
    }
}
